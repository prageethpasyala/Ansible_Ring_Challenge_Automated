...
if [ ! -e "$file" ]; then
    ...
    servername="ubuntu@$pubsrv1"
    
    # read the log number from remote server
    result=`ssh -oStrictHostKeyChecking=no -i "ta-lab-key.pem" ''$servername'' "cat /var/www/html/log.txt"`
    echo "Current server log number is : " $result  
    ((result=result+1))      
    # write result to remote server - log file : (replace text)
    echo $result | ssh -oStrictHostKeyChecking=no -i "ta-lab-key.pem" $servername -T "cat > /var/www/html/log.txt"
    ...
    echo "Current log number   : " $result | ssh -oStrictHostKeyChecking=no -i "ta-lab-key.pem" $servername -T "cat >> /var/www/html/masterlog.txt"
    ip4=$(/sbin/ip -o -4 addr list eth0 | awk '{print $4}' | cut -d/ -f1)
    #echo "Log updated by server : " $ip4 | ssh -i "ta-lab-key.pem" $servername -T "cat >> /home/ec2-user/masterlog.txt"    
    echo "------------------------------>" + $timestamp " updated by :" $ip4 +"<---------------------" | ssh -oStrictHostKeyChecking=no -i "ta-lab-key.pem" $servername -T "cat >> /var/www/html/masterlog.txt"
